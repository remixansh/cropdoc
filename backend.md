...
## Risks And Gaps
- Contract drift risk between docs and runtime stream format.
- Model artifact naming mismatch risk across scripts/runtime loaders.
- Input validation is limited (file type/size and coordinate range checks are minimal).
- CORS policy is permissive for MVP convenience.
- Stream completion semantics rely mostly on connection close.
- Test suite is mostly smoke-style and needs stronger assertions.
